...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const KEYS = ['name', 'version', 'headline', 'badge', 'intro', 'logoText', 'logoUrl', 'title', 'favicon', 'hideNotice', 'color']

    /** Every field empty: stock GUI branding. */
    function blank() {
      return { name: '', version: '', headline: '', badge: '', intro: '', logoText: '', logoUrl: '', title: '', favicon: '', hideNotice: '', color: '' }
    }
    ...
    function buildCss(cfg) {
      ...
      // Brand color: override the DSH brand CSS variables so the sidebar icons
      // and accent elements follow the configured brand color. Other UI plugins
      // (e.g. skin-center) may override these — our rule is a baseline, not a
      // guarantee.
      if (cfg.color !== '') {
        const c = cfg.color
        css += ':root{--dsw-brand-primary:' + c + ' !important;'
          + '--dsw-alias-label-primary-bluish:' + c + ' !important;'
          + '--dsw-alias-interactive-bg-hover:' + c + ' !important;'
          + '--dsw-alias-state-business-tertiary:' + c + '14 !important;}'
      }
      return css
    }
    ...
    function apply(ctx) {
      ...
      function BrandSection() {
        ...
        const colorField = (key, label) => h('div', { className: 'dsh-brand-field' },
          ...
          h('div', { className: 'dsh-brand-colorrow' },
            h('input', {
              ...
              value: /^#[0-9a-fA-F]{6}$/.test(form[key]) ? form[key] : '#ff6b1a',
              onChange: (ev) => updateField(key, ev !== null && typeof ev === 'object' && ev.target ? String(ev.target.value) : ''),
            }),
            h('input', {
              className: 'dsh-brand-field-input',
        ...
        const write = (cfg, pending, done) => {
          ...
          const imageFields = ['logoUrl', 'favicon', 'name', 'version', 'headline', 'badge', 'intro']
          const payload = {}
        ...
        return h('div', { className: 'dsh-brand-form' },
          h('div', { className: 'dsh-brand-hint' },
            '一键替换界面品牌信息，配置保存在 $DSH_HOME/dsh-brand.json，可直接分发给二改部署。Hero 三项（主标题 / 徽标 / 简介）任一填写即接管新会话页头部；全部留空显示官方默认。favicon 替换浏览器标签页图标；勾选「隐藏内测声明」可屏蔽 DSH 启动弹窗。品牌主色覆盖 DSH 主题变量，但其他 UI 插件（如皮肤中心）可能会覆盖这些样式。'),
