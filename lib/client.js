...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const KEYS = ['name', 'version', 'headline', 'badge', 'intro', 'logoText', 'logoUrl', 'title', 'favicon', 'sendIcon', 'stopIcon', 'hideNotice', 'thinkText', 'colorEnabled', 'color']

    /** Every field empty: stock GUI branding; color feature remains enabled. */
    function blank() {
      return { name: '', version: '', headline: '', badge: '', intro: '', logoText: '', logoUrl: '', title: '', favicon: '', sendIcon: '', stopIcon: '', hideNotice: '', thinkText: '', colorEnabled: 'true', color: '' }
    }
    ...
    function nameElement(cfg) {
      return h('span', { className: 'dsh-brand-namewrap' },
        h('span', { className: 'dsh-brand-namemain' },
        ...
        cfg.version !== '' ? (isImageValue(cfg.version)
          ? h('img', { className: 'dsh-brand-versionimg', src: ensureDataUrl(cfg.version), alt: 'version' })
          : h('span', { className: 'dsh-brand-versionbadge' }, cfg.version)) : null)
    }
    ...
    function buildCss(cfg) {
      ...
      css += '.dsh-brand-namewrap{display:inline-flex;flex:1 1 auto;align-items:center;gap:6px;width:100%;max-width:100%;min-width:0;overflow:hidden;white-space:nowrap;}'
      css += '.dsh-brand-namemain{display:flex;flex:1 1 0;align-items:center;min-width:0;max-width:100%;height:22px;overflow:hidden;}'
      css += '.dsh-brand-nametext{display:block;min-width:0;max-width:100%;overflow:hidden;text-overflow:ellipsis;font-size:17px;letter-spacing:0;white-space:nowrap;color:var(--dsw-alias-label-primary);}'
      css += '.dsh-brand-nameimg{display:block;flex:0 1 auto;width:auto;height:auto;max-width:100%;max-height:22px;object-fit:contain;object-position:left center;}'
      css += '.dsh-brand-versionbadge{display:inline-flex;flex:0 0 auto;box-sizing:border-box;align-items:center;justify-content:center;height:16px;min-width:max-content;padding:0 4px;border-radius:3px;color:var(--dsw-alias-label-primary-inverted);background:var(--dsw-alias-label-primary);font-family:var(--ds-font-family-code);font-size:8px;font-weight:500;line-height:16px;white-space:nowrap;overflow:visible;}'
      css += '.dsh-brand-versionimg{display:block;flex:0 0 auto;width:auto;height:16px;max-width:none;object-fit:contain;}'
      ...
      if (cfg.thinkText !== '') {
        ...
        css += 'div[data-variant="think"] span[class*="title"]::after{content:' + t + ';font-size:14px;'
          + (brandColor !== '' ? 'color:' + brandColor + ';' : 'color:var(--dsw-alias-label-primary);') + '}'
      }
      return css
    }
    ...
    function apply(ctx) {
      ...
      function BrandSection() {
        ...
        return h('div', { className: 'dsh-brand-form' },
          ...
          field('version', '版本徽标（文字 / 图片 URL / SVG）', '如：v1.0.0 或 https://img.shields.io/…'),
          ...
          h('div', { className: 'dsh-brand-scope' },
            '作用范围：侧边栏操作图标、项目文件夹、Think / Deep diving 文字、发送按钮与停止按钮。关闭后恢复系统原配色，已填写颜色不会被清除。'),
          h('div', { className: 'dsh-brand-field dsh-brand-field-row' },
