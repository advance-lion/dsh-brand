...
function sanitize(input) {
  ...
  // Auto-convert raw SVG markup to data URLs for image fields.
  if (/<svg/i.test(out.logoUrl)) out.logoUrl = ('data:image/svg+xml,' + encodeURIComponent(out.logoUrl)).slice(0, LIMITS.logoUrl)
  if (/<svg/i.test(out.favicon)) out.favicon = ('data:image/svg+xml,' + encodeURIComponent(out.favicon)).slice(0, LIMITS.favicon)
  return out
}
